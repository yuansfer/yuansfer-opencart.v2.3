...
class ControllerExtensionPaymentYuansfer extends Controller {
    ...
    public function index() {
        ...
        $data['testmode'] = $this->config->get('yuansfer_test');
            ...
            if (isset($this->request->post['status'])) {
                ...
                switch ($this->request->post['status']) {
                    case 'success':
                        ...
                        $order_status_id = $this->config->get('yuansfer_completed_status_id');
                        break;
                    case 'pending':
                        $status = 'pending';
                        
                        $order_status_id = $this->config->get('yuansfer_pending_status_id');
                        break;
                    case 'failed':
                        $status = 'failed';
                        
                        $order_status_id = $this->config->get('yuansfer_failed_status_id');
                        break;
                    case 'init':
                        $status = 'init';
    ...
    private function sign($data) {
        ...
        return md5($sign . md5($this->config->get('yuansfer_token')));
    }
