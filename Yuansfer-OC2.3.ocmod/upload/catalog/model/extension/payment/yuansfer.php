...
class ModelExtensionPaymentYuansfer extends Model {
    public function getMethod($address, $total) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('yuansfer_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('yuansfer_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('yuansfer_geo_zone_id')) {
            $status = true;
        ...
        if (!$this->config->get('yuansfer_merchant_id') || !$this->config->get('yuansfer_store_id') || !$this->config->get('yuansfer_token')) {
            $status = false;
        }
        ...
        if ($status) {
            ...
            if ($selected) {
                $method_data = array(
                    ...
                    'terms'      => '',
                    'sort_order' => $this->config->get('yuansfer_sort_order')
                );
            }
        }
    ...
    public function getPaymentMethods() {
        ...
        foreach ($payment_methods as $payment_method) {
            if (is_array($this->config->get('yuansfer_payment_method')) && in_array($payment_method, $this->config->get('yuansfer_payment_method'))) {
